...
func (s *HttpService) TopArtistCollage(w http.ResponseWriter, r *http.Request) {
...
func (s *HttpService) TopAlbumsCollage(w http.ResponseWriter, r *http.Request) {
	session_cookie, err := r.Cookie("session_id")
	if err != nil {
		http.Error(w, err.Error(), http.StatusUnauthorized)
		return
	...
	var resp *core_pb.Image
	if collageParams.Random {
		resp, err = s.Core_service.GetRandomTopAlbumsCollage(context.Background(), assembledCollageParams)
	} else {
		resp, err = s.Core_service.GetTopAlbumsCollage(context.Background(), assembledCollageParams)
	}
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
	}
