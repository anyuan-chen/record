...
type Session_manager_server struct {
	Session_table map[string]*core_pb.Token
	session_manager_pb.UnimplementedSessionManagerServer
}

func (s *Session_manager_server) CreateSession(ctx context.Context, token *core_pb.Token) (*session_manager_pb.SessionID, error) {
	var uid string
	for {
		uid = uuid.New().String()
		if proto.Equal(token, &core_pb.Token{Token: nil}) {
			continue
		} else {
			break
		}
...
func (s *Session_manager_server) GetSession(ctx context.Context, session_id *session_manager_pb.SessionID) (*core_pb.Token, error) {
